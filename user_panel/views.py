...
def ajax_filter_products(request):
    ...
    if cat_obj and cat_obj.name.lower().replace(' ', '').replace('-', '') == 'giftsets':
        ...
        for var in unique_products.values():
            ...
            product_price_range = product_variants.aggregate(
            min_price=Min('price'),
            max_price=Max('price')
    )
            ...
            product_data.append({
                ...
                'min_price': float(product_price_range['min_price']) if product_price_range['min_price'] else None,
                'max_price': float(product_price_range['max_price']) if product_price_range['max_price'] else None,
...
def add_to_cart(request, product_id):
    ...
    # # Check if the last order is finished
    # if last_order and last_order.status not in ["completed", "awbassigned"]:
    #     return JsonResponse({"success": False, "review_required": True})
...
def search_suggestions(request):
    ...
    for product in matching_products:
        ...
        suggestions.append({
            ...
            'image': product.image1.url if product.image1 else '',
            'description': product.description[:100] if product.description else '',
            'url': f"{reverse('products_list')}?category={product.category.id}",
...
def subscription_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
              ...
              <body style="font-family: Arial, sans-serif; background:#f9f9f9; padding:20px;">
                <div style="max-width:600px; margin:auto; background:#fff; border-radius:10px; box-shadow:0 4px 8px rgba(0,0,0,0.1); padding:20px;">
                  ...
                  <p style="font-size:16px; color:#333;">
                    Hi <b>{subscription.email}</b>,<br><br>
                    Thank you for subscribing to <span style="color:#d63384;">Perfumavalley</span>! ✨
                  </p>
                  <p style="font-size:16px; color:#333;">
                    You'll now receive exclusive updates, special offers, and the latest fragrances straight to your inbox.  
                  </p>
            ...
            # Send Email
            try:
                msg = EmailMultiAlternatives(subject, text_content, settings.DEFAULT_FROM_EMAIL, [subscription.email])
                msg.attach_alternative(html_content, "text/html")
                msg.send()
            except Exception as e:
                pass  # We'll still show success to user
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                return JsonResponse({'success': False, 'errors': errors, 'error': 'Please check the form fields.'})
    ...
    return render(request, 'user_panel/home1.html', {'form': form})
